...
async def lifespan(app: FastAPI):
    ...
    if not os.environ.get("AGENTS_BASE"):
        logger.warning("AGENTS_BASE not set; using default path /opt/haic/agents. Set AGENTS_BASE env var to override.")
    yield
...
_AGENTS_BASE = Path(os.environ.get("AGENTS_BASE", "/opt/haic/agents"))
...
async def internal_agent_prompt(body: AgentPromptRequest, request: Request):
    ...
    logger.info(f"internal_agent_prompt: queued for {body.agent} at {timestamp} (priority={body.priority})")
    return {"status": "queued", "agent": body.agent, "timestamp": timestamp}
