import pytest
from semantic_grounding.analysis.early_warning import EarlyWarningAnalyzer
...
def test_detect_accuracy_first(analyzer):
    ...
    metrics = [
        ...
        {"generation": 1, "grounded_arc_accuracy": 0.6, "grounded_arc_perplexity": 10.2}, # Acc fails
        {"generation": 2, "grounded_arc_accuracy": 0.5, "grounded_arc_perplexity": 12.0}  # PPL fails
    ]
...
def test_detect_perplexity_first(analyzer):
    ...
    metrics = [
        {"generation": 0, "grounded_arc_accuracy": 0.8, "grounded_arc_perplexity": 10.0},
        {"generation": 1, "grounded_arc_accuracy": 0.79, "grounded_arc_perplexity": 11.0}, # PPL fails
        {"generation": 2, "grounded_arc_accuracy": 0.6, "grounded_arc_perplexity": 12.0}   # Acc fails
    ]
    result = analyzer.detect(metrics)
...
def test_detect_single_generation(analyzer):
    """(3) only one generation -> no detection"""
    metrics = [
        {"generation": 0, "grounded_arc_accuracy": 0.8, "grounded_arc_perplexity": 10.0}
    ]
    result = analyzer.detect(metrics)
...
def test_generate_report(analyzer):
    """(4) generate_report() returns a string with regime name and signature status"""
